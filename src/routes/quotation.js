...
router.get('/:idx', function(req, res, next) {
    ...
    const query = 'SELECT ' + table_name + '.*, ' + brand_table_name + '.brand_name, ' + model_table_name + '.model_name, ' + lineup_table_name + '.lineup_name, ' + trim_table_name + '.trim_name, ' + car_kind_table_name + '.kind_name ' + 
                'FROM ' + table_name + ' ' +
...
router.post('/list/:offset?', function(req, res, next) {
    ...
    if(req.body.date_type) {
        switch (req.body.date_type) {
            case '0':
                if(req.body.s_date) {
                    where_array.push(table_name + '.reg_date >= ' + req.body.s_date);
                }
            
                if(req.body.e_date) {
                    where_array.push(table_name + '.reg_date <= ' + req.body.e_date);
                }
                break;

            case '1':
                if(req.body.s_date) {
                    where_array.push(table_name + '.contract_date >= ' + req.body.s_date);
                }
            
                if(req.body.e_date) {
                    where_array.push(table_name + '.contract_date <= ' + req.body.e_date);
                }
                break;

            case '2':
                if(req.body.s_date) {
                    where_array.push(table_name + '.release_date >= ' + req.body.s_date);
                }
            
                if(req.body.e_date) {
                    where_array.push(table_name + '.release_date <= ' + req.body.e_date);
                }
                break;
    ...
    if(req.body.purchase_method) {
        where_array.push(table_name + '.purchase_method = ' + req.body.purchase_method);
    }
        ...
        switch (req.body.search_type) {
            case '0':
                if(req.body.search_text) {
                    where_array.push(table_name + '.client_name = \'' + req.body.search_text + '\'');
                }
                break;

            case '1':
                if(req.body.search_text) {
                    where_array.push(table_name + '.client_phone = \'' + req.body.search_text + '\'');
                }
                break;

            case '2':
                if(req.body.search_text) {
                    where_array.push(car_kind_table_name + '.kind_name = \'' + req.body.search_text + '\'');
                }
                break;
    ...
    const query =   'SELECT ' + table_name + '.*, ' + brand_table_name + '.brand_name, ' + model_table_name + '.model_name, ' + lineup_table_name + '.lineup_name, ' + car_kind_table_name + '.kind_name ' + 
                    'FROM ?? ' + 
                    'LEFT JOIN ' + brand_table_name + ' ON ' + table_name + '.brand_id = ' + brand_table_name + '.idx ' + 
                    'LEFT JOIN ' + model_table_name + ' ON ' + table_name + '.model_id = ' + model_table_name + '.idx ' + 
                    'LEFT JOIN ' + lineup_table_name + ' ON ' + table_name + '.lineup_id = ' + lineup_table_name + '.idx ' + 
